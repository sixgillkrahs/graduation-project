...
interface EventModalProps {
  open: boolean;
  onClose: () => void;
  selectedEvent?: EventClickArgs;
}

const EventModal = memo(({ open, onClose, selectedEvent }: EventModalProps) => {
  ...
  const { data: scheduleDetail, isLoading: isLoadingDetail } =
    useGetScheduleById(selectedEvent?._def.publicId || "");
  ...
  const onSubmit = async (data: CreateScheduleRequest) => {
    if (selectedEvent) {
      await updateSchedule({ id: selectedEvent._def.publicId, data });
    } else {
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        initialFocus
                        mode="range"
                        defaultMonth={selectedEvent?._instance.range?.start}
                        // selected={selectedEvent?._instance.range}
