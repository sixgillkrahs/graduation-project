...
const Review = ({ propertyId }: ReviewProps) => {
  const dispatch = useDispatch();
  const router = useRouter();
  const { mutateAsync: updateProperty, isPending: isUpdatingProperty } =
    useUpdateProperty();
  ...
  const onPublish = (formData: ListingFormData) => {
    updateProperty(
      ...
      {
        onSuccess: () => {
          toast.success("Listing updated successfully! Wait for approval.");
                  ...
                  <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 z-10 px-4 overflow-x-auto">
                    {virtualTourUrls.map((url, idx) => (
                      <button
                        ...
                        className={`w-16 h-10 rounded border-2 overflow-hidden transition-all shrink-0 ${
                          idx === active360Index
